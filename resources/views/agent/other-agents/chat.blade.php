...
@section('content')
<div class="flex flex-col h-[calc(100vh-8rem)] bg-gray-900 rounded-xl shadow-sm border border-gray-800 overflow-hidden">
    {{-- Header --}}
    <div class="px-6 py-4 border-b border-gray-800 flex items-center justify-between bg-gray-800">
        <div class="flex items-center gap-4">
            <a href="{{ route('agent.agents.index') }}" class="p-2 -ml-2 text-gray-400 hover:text-gray-400 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                </svg>
            </a>
            <div class="relative">
                <div class="w-10 h-10 rounded-full bg-gray-200 flex items-center justify-center text-lg font-bold text-gray-400">
            ...
            <div>
                <h2 class="text-sm font-semibold text-gray-100">{{ $otherUser->name }}</h2>
    ...
    {{-- Messages Area --}}
    <div id="internal-chat-container" class="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-800/50">
        @foreach($messages as $msg)
            <div class="flex {{ $msg->sender_id === auth()->id() ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-[70%] rounded-2xl px-4 py-2.5 text-sm shadow-sm 
                    {{ $msg->sender_id === auth()->id() ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-gray-900 text-gray-100 border border-gray-800 rounded-tl-none' }}">
        ...
        <form action="{{ route('agent.agents.message', $otherUser->id) }}" method="POST" data-ajax-form id="internal-chat-form">
            ...
            <div class="flex items-center gap-2">
                <input type="text" name="message" required placeholder="Type a message..." autocomplete="off"
                    class="flex-1 rounded-full border-gray-700 focus:border-blue-500 focus:ring-blue-500 text-sm py-2 px-4 shadow-sm bg-gray-800">
