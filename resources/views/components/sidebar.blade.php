...
<div id="app-sidebar" class="fixed inset-y-0 left-0 -translate-x-full transition-transform duration-300 ease-in-out lg:static lg:translate-x-0 w-72 max-w-[85vw] lg:w-64 lg:max-w-none flex-shrink-0 bg-slate-900 lg:bg-slate-900/40 border-r border-slate-700/50 backdrop-blur-xl text-slate-300 flex flex-col shadow-2xl z-40 lg:z-20">
    ...
    <nav id="sidebar-nav" class="flex-1 overflow-y-auto py-3">
        <ul class="space-y-0.5">
            ...
            <li>
                <a href="{{ route('agent.chats.index') }}" class="flex items-center justify-between px-4 py-2.5 hover:bg-slate-700/50 rounded-xl mx-2 transition-all duration-200 group {{ request()->routeIs('agent.chats.*') ? $navActive : $navIdle }}">
                    ...
                    <span id="unread-chat-counter" class="hidden inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#6366F1]/20 text-[#818CF8] border border-[#6366F1]/30 leading-none shadow-sm transition-transform"></span>
                </a>
            </li>
            <li>
                <a href="{{ $isAdmin ? route('admin.queue.index') : route('agent.queue.index') }}" class="flex items-center justify-between px-4 py-2.5 hover:bg-slate-700/50 rounded-xl mx-2 transition-all duration-200 group {{ request()->routeIs('admin.queue.*') || request()->routeIs('agent.queue.*') ? $navActive : $navIdle }}">
                    ...
                    <span id="sidebar-queue-count" class="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#6366F1]/20 text-[#818CF8] border border-[#6366F1]/30 leading-none shadow-sm transition-transform {{ $pendingQueueCount > 0 ? '' : 'hidden' }}">
            ...
            <li>
                <a href="{{ route('admin.chats.index') }}" class="flex items-center justify-between px-4 py-2.5 hover:bg-slate-700/50 rounded-xl mx-2 transition-all duration-200 group {{ request()->routeIs('admin.chats.*') ? $navActive : $navIdle }}">
                    <div class="flex items-center">
                        <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <span id="monitor-unread-counter" class="hidden inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#6366F1]/20 text-[#818CF8] border border-[#6366F1]/30 leading-none shadow-sm transition-transform"></span>
                </a>
            </li>
            @endif
            ...
            <li>
                <a href="#" class="{{ $navBase }} {{ request()->routeIs('crm.pipelines.*') ? $navActive : $navIdle }}"> <!-- TODO: Add route('crm.pipelines.index') -->
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h16M4 18h16"></path>
                    </svg>
                    <span class="text-sm font-semibold">Pipelines</span>
                </a>
            </li>
            <li>
            ...
            <li>
                <a href="#" class="{{ $navBase }} {{ request()->routeIs('crm.tasks.*') ? $navActive : $navIdle }}"> <!-- TODO: Add route('crm.tasks.index') -->
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- ─────────────  REPORTS  ───────────── --}}
            <li class="px-5 pt-4 pb-1">
                <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">Reports</span>
            </li>
            <li>
                <a href="#" class="{{ $navBase }} {{ request()->routeIs('reports.*') ? $navActive : $navIdle }}"> <!-- TODO: Add route('reports.index') -->
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <li>
                <a href="#" class="{{ $navBase }} {{ request()->routeIs('analytics.*') ? $navActive : $navIdle }}"> <!-- TODO: Add route('analytics.index') -->
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {{-- ─────────────  SETTINGS  ───────────── --}}
            <li class="px-5 pt-4 pb-1">
                <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">Settings</span>
            </li>
            @if($canChat)
            <li>
                <a href="{{ route('agent.agents.index') }}" class="flex items-center justify-between px-4 py-2.5 hover:bg-slate-700/50 rounded-xl mx-2 transition-all duration-200 group {{ request()->routeIs('agent.agents.*') ? $navActive : $navIdle }}">
                    ...
                    <span id="team-unread-badge" class="inline-flex items-center justify-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#6366F1]/20 text-[#818CF8] border border-[#6366F1]/30 leading-none shadow-sm transition-transform {{ $totalUnreadInternal > 0 ? '' : 'hidden' }}">
            ...
            <li>
                <a href="#" class="{{ $navBase }} {{ request()->routeIs('admin.integrations.*') ? $navActive : $navIdle }}"> <!-- TODO: Add route('admin.integrations.index') -->
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"></path>
                    </svg>
                    <span class="text-sm font-semibold">Integrations</span>
                </a>
            </li>
            <li>
