@extends('layouts.app')
@section('header_title', 'CRM · New Product')
...
@php
    ...
    $initialTiers = collect(old('tiers', []))
        ->map(fn ($t) => ['min_quantity' => $t['min_quantity'] ?? '', 'unit_price' => $t['unit_price'] ?? ''])
        ->values();
@endphp

@section('content')
<x-crm.page-header title="New Product" subtitle="Add a product to the catalogue with optional volume pricing." />

<form method="POST" action="{{ route('crm.products.store') }}" class="max-w-3xl space-y-6">
    @csrf

    <x-crm.panel title="Product Details">
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
            <x-crm.field label="Name" name="name" :required="true" class="sm:col-span-2">
                <input type="text" id="name" name="name" value="{{ old('name') }}" class="{{ $inputClass }}" placeholder="e.g. Kraft Mailer Box">
            </x-crm.field>
            ...
            <x-crm.field label="Type" name="type" :required="true">
                <select id="type" name="type" class="{{ $inputClass }}">
                    ...
                    <option value="{{ $case->value }}" @selected(old('type') === $case->value)>{{ $case->getLabel() }}</option>
                    @endforeach
                </select>
            </x-crm.field>

            <x-crm.field label="Material" name="material">
                <input type="text" id="material" name="material" value="{{ old('material') }}" class="{{ $inputClass }}" placeholder="e.g. 3-ply corrugated">
            </x-crm.field>

            <x-crm.field label="Size Options" name="size_options" class="sm:col-span-2">
                <input type="text" id="size_options" name="size_options" value="{{ old('size_options') }}" class="{{ $inputClass }}" placeholder="e.g. S / M / L, or 20x15x10 cm">
            </x-crm.field>

            <x-crm.field label="MOQ" name="moq" :required="true">
                <input type="number" id="moq" name="moq" value="{{ old('moq', 1) }}" min="1" step="1" class="{{ $inputClass }}">
            </x-crm.field>

            <x-crm.field label="Base Price (PKR)" name="base_price" :required="true">
                <input type="number" id="base_price" name="base_price" value="{{ old('base_price') }}" min="0" step="0.01" class="{{ $inputClass }}" placeholder="0.00">
            </x-crm.field>
        </div>
    </x-crm.panel>
    ...
    <div class="flex items-center gap-3">
        <button type="submit" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#6366F1] hover:bg-[#4F46E5] transition-all shadow-lg shadow-[#6366F1]/20">Create Product</button>
