@extends('layouts.app')
@section('header_title', 'New Quotation')

@section('content')
@php($company = $deal->lead?->contact?->company)
@php($contact = $deal->lead?->contact)
<x-crm.page-header title="New Quotation" subtitle="{{ $company?->name ?? 'Deal #' . $deal->id }} — version {{ $version }}">
    <x-slot:actions>
        <a href="{{ route('crm.deals.show', $deal->id) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-300 border border-slate-700 hover:bg-slate-800 transition-all">Back to Deal</a>
    </x-slot:actions>
</x-crm.page-header>

<form method="POST" action="{{ route('crm.quotations.store') }}"
      x-data="quotationForm(@js($products->map(fn($p) => ['id' => $p->id, 'base_price' => (float) $p->base_price])->values()))">
    @csrf
    ...
    <x-crm.panel title="Details">
        <div class="p-6 grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <x-crm.field label="Version">
                <input type="text" value="{{ $version }}" readonly
                       class="w-full bg-slate-900/50 border border-slate-700 rounded-xl px-3 py-2 text-sm text-slate-400 outline-none">
            </x-crm.field>
        </div>
    </x-crm.panel>
    ...
    <div class="mt-6 flex items-center gap-3">
        <button type="submit" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#6366F1] hover:bg-[#4F46E5] transition-all shadow-lg shadow-[#6366F1]/20">Create Quotation</button>
        <a href="{{ route('crm.deals.show', $deal->id) }}" class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-300 border border-slate-700 hover:bg-slate-800 transition-all">Cancel</a>
    </div>
</form>

<script>
    function quotationForm(products) {
        return {
            products: products,
            rows: [{ _key: Date.now(), product_id: '', quantity: 1, unit_price: '' }],
            addRow() {
